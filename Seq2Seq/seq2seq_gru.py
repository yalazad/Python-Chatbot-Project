...
def test_step(val_inp, val_targ, encoder_hidden):
    loss = 0
    encoder_output, encoder_hidden = encoder(val_inp, encoder_hidden)
...
def evaluate(sentence):
    ...
    encoder_out, encoder_hidden = encoder(enc_inputs, hidden)
...
input_tensor_train, input_tensor_val, response_tensor_train, response_tensor_val = train_test_split(input_tensor, response_tensor, test_size=0.2)
...
# Create encoder
encoder = gru_encoder.Encoder(vocab_inp_size, embedding_dim, units, BATCH_SIZE)

# Create attention layer
attention_layer = bahdanau.BahdanauAttention(10)

# Create decoder
decoder = gru_decoder.Decoder(vocab_resp_size, embedding_dim, units, BATCH_SIZE)
...
val_accuracy = tf.keras.metrics.SparseCategoricalAccuracy('vals_accuracy')
...
if manager.latest_checkpoint:    
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for (val_batch, (val_inp, val_targ)) in enumerate(val_dataset.take(val_steps_per_epoch)):
            val_batch_loss = test_step(val_inp, val_targ, encoder_hidden)
